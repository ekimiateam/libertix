...
require_build_dependencies() {
    ...
    for command in debootstrap mksquashfs xorriso mmd mcopy mkfs.vfat sha256sum; do
...
configure_grub_efi() {
    ...
    mmd -i "$WORKDIR/iso_build/boot/grub/efi.img" ::/EFI ::/EFI/BOOT ::/EFI/debian ::/EFI/LibertixInstaller
