...
load_libertix_installation_plan() {
    ...
    while IFS=$'\t' read -r name value; do
        case "$name" in
            INSTALLATION_PLAN_ID|INSTALLATION_FIRMWARE|LANGUAGE_CODE|SYSTEM_LANG|KEYBOARD_LAYOUT|KEYBOARD_MODEL|TIMEZONE|\
            USERNAME|PASSWORD_HASH|COMPUTER_NAME|ISO_FILENAME|ISO_URL|ISO_WINDOWS_PATH|ISO_SHA256|LINUX_SIZE_GB|\
            TARGET_DISK_NUMBER|TARGET_DISK_UNIQUE_ID|TARGET_DISK_SIZE_BYTES|TARGET_LOGICAL_SECTOR_SIZE_BYTES|\
            WINDOWS_PARTITION_NUMBER|WINDOWS_PARTITION_OFFSET_BYTES|WINDOWS_BOOT_PARTITION_NUMBER|\
            ...
            SHARE_LINUX_FILES_IN_WINDOWS|WINDOWS_PROFILES_JSON_BASE64)
                export "$name=$value"
                ;;
            *)
