...
load_libertix_installation_plan() {
    ...
    while IFS=$'\t' read -r name value; do
        case "$name" in
            INSTALLATION_PLAN_ID|INSTALLATION_FIRMWARE|DISTRIBUTION_ID|DISTRIBUTION_NAME|DISTRIBUTION_OS_RELEASE_ID|DISTRIBUTION_GRUB_DISPLAY_NAME|DISTRIBUTION_GRUB_ICON|DISTRIBUTION_SECURE_BOOT_MICROSOFT_AUTHORITIES|\
            LANGUAGE_CODE|SYSTEM_LANG|KEYBOARD_LAYOUT|KEYBOARD_VARIANT|KEYBOARD_MODEL|TIMEZONE|\
            USERNAME|PASSWORD_HASH_WINDOWS_PATH|COMPUTER_NAME|ISO_FILENAME|ISO_URL|ISO_WINDOWS_PATH|ISO_SHA256|LINUX_SIZE_GB|\
            TARGET_DISK_NUMBER|TARGET_DISK_UNIQUE_ID|TARGET_DISK_PARTITION_TABLE_ID|TARGET_DISK_SIZE_BYTES|TARGET_LOGICAL_SECTOR_SIZE_BYTES|\
            ...
            INSTALLER_FINAL_OFFSET_BYTES|INSTALLER_RESIZE_MODE|INSTALLER_FINAL_SIZE_BYTES|INSTALLER_STAGING_SIZE_BYTES|\
            WINDOWS_BITLOCKER_STATE|EXPECTED_PARTITION_STYLE|\
            RECOVERY_PARTITION_NUMBER|RECOVERY_PARTITION_OFFSET_BYTES|RECOVERY_PARTITION_SIZE_BYTES|\
            RECOVERY_ROOT_WINDOWS|RECOVERY_RUN_ID|LOW_MEMORY_MODE|SECURE_BOOT_ENABLED|TRUSTED_MICROSOFT_UEFI_AUTHORITIES|LIVE_MINIMUM_MEMORY_MIB|INSTALLER_ALIGNMENT_BYTES|SHARE_WINDOWS_FILES_IN_LINUX|\
