...
def child(hive: Any, node: int, name: str) -> int:
...
def value_bytes(hive: Any, node: int | None, value_name: str = "Element") -> bytes | None:
    if not node:
        return None
...
def set_guid_list(hive: Any, elements: int, element_name: str, guids: list[str]) -> None:
...
def main() -> int:
    ...
    for obj in hive.node_children(objects):
        ...
        if is_libertix_temporary_entry(desc, path):
            targets.append(guid)
