...
prepare_offline_ntfs_resize_or_die() {
    ...
    local source_bitlocker="${ALLOCATION_SOURCE_BITLOCKER_STATE:-$WINDOWS_BITLOCKER_STATE}"
    ...
    assert_not_mounted_or_open "$source_part"
    assert_recovery_unchanged_or_die
    allocation_source_filesystem_matches_manifest "$source_part" || \
        die "offline NTFS resize source filesystem no longer matches the installation plan"
    case "$source_bitlocker" in
    ...
    [ "$INSTALLER_FINAL_OFFSET_BYTES" -gt "$source_offset" ] 2>/dev/null || \
    ...
    final_windows_size=$((final_windows_end - source_offset))
    ...
    current_windows_size="$(blockdev --getsize64 "$source_part" 2>/dev/null || echo 0)"
    current_windows_end=$((source_offset + current_windows_size))
    ...
    run_logged ntfsresize --force --size "$final_windows_size" "$source_part" || \
