...
if (-not (Test-Path -LiteralPath $exe -PathType Leaf)) {
    throw ("LinuxGate.exe local est introuvable: " + $exe)
}
...
if ($LASTEXITCODE -ne 0) {
    throw ("Création tâche planifiée LinuxGate échouée; sortie=" + ($createOutput -join " | "))
}

$runOutput = schtasks.exe /Run /TN $taskName 2>&1
if ($LASTEXITCODE -ne 0) {
    throw ("Lancement tâche planifiée LinuxGate échoué; sortie=" + ($runOutput -join " | "))
}
...
if (-not $process) {
    throw "LinuxGate ne tourne pas après lancement administrateur"
}
