...
if (-not (Test-Path -LiteralPath $exe -PathType Leaf)) {
    throw "LinuxGate.exe local est introuvable"
}
...
if ($mode -eq "prepare") {
    ...
    Get-Process -Name "LinuxGate" -ErrorAction SilentlyContinue |
        Where-Object { $_.SessionId -eq $interactive.SessionId } |
        Stop-Process -Force

    # LinuxGate demande l'élévation dans son manifeste. Pour la validation visuelle
    ...
    $shortcut = Join-Path $desktop "LinuxGate.lnk"
...
if ($mode -eq "verify") {
    ...
    try {
        ...
        do {
            ...
            $process = Get-Process -Name "LinuxGate" -ErrorAction SilentlyContinue |
        ...
        if (-not $process) {
            throw "LinuxGate ne démarre pas dans la session graphique"
        }
